...
void grid_points() {
	...
	for (int t = 0; t < allStreamlines.size(); t++) {
		...
		if (tmp.scalar >= 3000) tmp.scalar = 2000;
		...
		if (checkt == true && tmp.scalar >= 2000) {
			//if (seedingCurve2.size() == 10) checkt = false;
	...
	printVolume2("../src/output/gridpoints.vts", allGridPoints);
	printVolume2("../src/output/gradients.vts", allGradient);
